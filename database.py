...
# ── Proposal Intelligence (migrations/015_proposal_intelligence.sql) ──────────
# PI-1, Proposal Intelligence. Every write here is privileged (service-role)
# and must only ever be reached through tenancy.py's authorization-boundary
# wrappers (require_bid_access() first) -- see proposal_intelligence.py for
...
def get_proposal_package_snapshots(bid_id: int) -> list[dict]:
    """Full history, most recent version first."""
    return _rows(get_client().table("proposal_package_snapshots").select("*")
...
def get_proposal_intelligence_run(run_id: int) -> dict | None:
    return _one(get_client().table("proposal_intelligence_runs").select("*")
