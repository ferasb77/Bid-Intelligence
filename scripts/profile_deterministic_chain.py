"""Function-level profiling of the three measured deterministic hotspots
(Opportunity Intelligence Publication, Executive Opportunity Understanding,
Executive Opportunity Brief) plus the full Evidence->Pack replay, against
the frozen authoritative Bank of Canada lineage (tag bid-intelligence-rc2).

Read-only with respect to production code. Zero LLM calls. Uses cProfile to
break each stage's wall time into function-level blocks, and pstats to
report the top cumulative-time contributors per stage.
"""
import cProfile
import io
import json
...
def profile_call(label, fn):
    ...
    (OUT / f"profile_{label}.txt").write_text(stream.getvalue(), encoding="utf-8")
    stream2 = io.StringIO()
    stats2 = pstats.Stats(pr, stream=stream2).sort_stats("tottime")
    stats2.print_stats(25)
    (OUT / f"profile_{label}_tottime.txt").write_text(stream2.getvalue(), encoding="utf-8")
    stream3 = io.StringIO()
    stats3 = pstats.Stats(pr, stream=stream3)
    stats3.print_callers("_canonical")
    stats3.print_callers("_resolve_one")
    stats3.print_callers("^_json$")
    stats3.print_callers("_find_context_object")
    stats3.print_callers("resolve_governed_reference")
    (OUT / f"profile_{label}_callers.txt").write_text(stream3.getvalue(), encoding="utf-8")
    print(f"{label}: {elapsed:.4f}s wall (profiled)")
    return result, elapsed


oi_publication, t_oi_pub = profile_call("opportunity_intelligence_publication", lambda: validate_opportunity_intelligence_publication(
...
understanding, t_eou = profile_call("executive_opportunity_understanding", lambda: validate_executive_opportunity_understanding(
    build_executive_opportunity_understanding(ExecutiveUnderstandingInput(OPPORTUNITY_ID, oi_publication))))

eob, t_eob = profile_call("executive_opportunity_brief", lambda: validate_executive_opportunity_brief(
    build_executive_opportunity_brief(understanding)))

print()
print("SUMMARY")
print("Opportunity Intelligence Publication:", round(t_oi_pub, 4), "s")
print("Executive Opportunity Understanding:", round(t_eou, 4), "s")
print("Executive Opportunity Brief:", round(t_eob, 4), "s")
print("brief_id:", eob.brief_id)
