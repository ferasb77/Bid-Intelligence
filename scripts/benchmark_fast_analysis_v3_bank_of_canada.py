"""FAST ANALYSIS V3 -- one live benchmark against the authoritative
16-document Bank of Canada RFP 2026-026 corpus, concurrency=2, following the
v1/v2 architecture unchanged except for bounded split-on-truncation recovery
replacing v2's ineffective same-content retry, plus the reverted Insurance
prompt experiment and the minimal pricing-consequence schema widening
documented in FAST_ANALYSIS_V3_IMPLEMENTATION_REPORT.md.

NON_AUTHORITATIVE_TELEMETRY_RUN. Does not touch, rerun, or overwrite any
Deep Verify, V1, or V2 artifact. V1 and V2 remain the performance/fidelity
controls:
  ...
  V2 run: fastanalysis-v2-boc-2026-026-20260913T120342Z-147f75
    (15 calls, 4 retries, 52,504 input / 39,597 output tokens, 181.478s,
     87.8% procurement fidelity on the reconciled methodology)
...
RUN_ID = f"fastanalysis-v3-boc-2026-026-{datetime.now(timezone.utc):%Y%m%dT%H%M%SZ}-{secrets.token_hex(3)}"
...
print("FAST ANALYSIS V3 benchmark:", RUN_ID)
...
print()
print("FAST ANALYSIS V3 TOTALS")
print(json.dumps(totals, indent=2, sort_keys=True, default=str))
print()
print("Persisted at:", OUT)

# ---- build the Fast Analysis V3 PDF via the shared renderer ----
...
fast_pdf_path = ROOT / "BANK_OF_CANADA_RFP_2026_026_BID_INTELLIGENCE_PREVIEW_FAST_V3.pdf"
...
print("Fast Analysis V3 PDF written to:", fast_pdf_path, f"({report_seconds:.3f}s)")
