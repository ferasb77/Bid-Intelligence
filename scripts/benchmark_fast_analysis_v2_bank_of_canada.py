"""FAST ANALYSIS V2 -- one live benchmark against the authoritative
16-document Bank of Canada RFP 2026-026 corpus, concurrency=2, following the
v1 architecture unchanged except for the targeted retry-trigger and
report-adapter fidelity fixes documented in FAST_ANALYSIS_V2_IMPLEMENTATION_REPORT.md.

NON_AUTHORITATIVE_TELEMETRY_RUN. Does not touch, rerun, or overwrite any
Deep Verify or Fast Analysis V1 artifact. V1 remains the performance/
fidelity control:
  V1 run: fastanalysis-v1-boc-2026-026-20260913T094132Z-5de413
    (12 calls, 1 retry, 37,438 input / 27,418 output tokens, 107.543s,
     85.0% procurement fidelity)
...
RUN_ID = f"fastanalysis-v2-boc-2026-026-{datetime.now(timezone.utc):%Y%m%dT%H%M%SZ}-{secrets.token_hex(3)}"
...
print("FAST ANALYSIS V2 benchmark:", RUN_ID)
...
print()
print("FAST ANALYSIS V2 TOTALS")
print(json.dumps(totals, indent=2, sort_keys=True, default=str))
print()
print("Persisted at:", OUT)

# ---- build the Fast Analysis V2 PDF via the shared renderer ----
...
fast_pdf_path = ROOT / "BANK_OF_CANADA_RFP_2026_026_BID_INTELLIGENCE_PREVIEW_FAST_V2.pdf"
...
print("Fast Analysis V2 PDF written to:", fast_pdf_path, f"({report_seconds:.3f}s)")
