...
This branch resolves the root cause by removing single-pass monolithic extraction for substantial documents. It introduces deterministic marker-aware document chunking, conservative factual aggregation, an automated Coverage Guard with bounded recovery, and strict sampling determinism (`temperature=0.0`).
...
2. **Non-Deterministic Sampling Defaults:**
   The Anthropic API invocation previously used default sampling parameters, allowing variance between runs on identical inputs.
...
### 3.1 Deterministic Marker-Aware Chunking
...
- Preprocessing already embeds deterministic source markers:
...
- If an individual block exceeds `max_chunk_chars`, it is subdivided on newline boundaries while inheriting the leading source marker to maintain provenance attribution.
- Short documents (length $\le 16,000$ characters) proceed in a single chunk.

### 3.2 Model Determinism
- `_extract_chunk_facts()` enforces:
  - `temperature=0.0`
  - Explicit model: `claude-haiku-4-5-20251001`
  - Fixed `max_tokens=8000`

### 3.3 Deterministic Aggregation Layer
...
- Combines facts across 8 procurement families:
  ...
  2. `requirements`: Deduplicated using normalized material content, category, and reference `(norm_desc, cat.lower(), rfso.lower())`. Requirements sharing the same local `req_id` (e.g., `M1` generated independently in Chunk 1 and Chunk 2) do **not** collapse if their descriptions differ. If descriptions match, physical `source_refs` are merged.
...
### 3.4 Coverage Guard & Bounded Recovery
...
- **Recovery:** Triggers narrower chunking (`max_chunk_chars=8000`) to force focused extraction on dense sections.
...
## 4. Known Benchmark Replay Results
...
- **Zero Silent Drop:** 0 of 6 runs returned zero decision-critical coverage for either document.
- **Main ITT:** Main ITT consistently yielded 26–49 requirements, 13–16 dates, 13 evaluation criteria, and 16–19 submission rules (compared to 0 in both Attempt 1 and Attempt 2).
- **Annex 2 PSQ:** Annex 2 consistently yielded 21–30 requirements (compared to 0 in Attempt 2).
- Neither document required fallback recovery because first-pass chunked extraction extracted all required fact families (`VERIFIED_ADEQUATE`).
...
Created [`tests/test_stage_a_extraction_reliability.py`](file:///C:/Users/feras/Documents/Projects/Bid-Intelligence/tests/test_stage_a_extraction_reliability.py) covering 17 tests:
...
- `tests/test_stage_a_extraction_reliability.py`: **17 / 17 passed**
...
- Full unit test discovery (`tests/test_*.py`): **256 / 256 passed**
- Grand total suite across unit, smoke, and integration: **274 discovered / 273 passed / 1 skipped (live AI) / 0 failed**
...
Stage A extraction is now deterministic, marker-aware, and guarded against silent fact omission. Repeated runs on substantial documents produce stable, comprehensive procurement facts across all core fact families.
